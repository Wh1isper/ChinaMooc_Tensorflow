...
class Vgg16():
    def __init__(self, vgg16_path=None):
        # 初始化，从vgg16.npy中读取训练好的神经网络
        if vgg16_path is None:
            vgg16_path = os.path.join(os.getcwd(), "vgg16.npy") 
            self.data_dict = np.load(vgg16_path, encoding='latin1').item() 
        # 得到self.data_dict即为全体列表
    def forward(self, images):
        # 还原前向传播的神经网络，输入是224*224*3的图片
        print("build model started")
        start_time = time.time()    # 记录用时
        rgb_scaled = images * 255.0     # 像素点反归一化
